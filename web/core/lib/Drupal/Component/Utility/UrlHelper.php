...
class UrlHelper {
  ...
  /**
   * Convert an url query string to an array.
   *
   * The parse_str method contains an issue with query parameter names
   * containing a dot. Having a dot in a query parameter name is valid, so
   * we hex encode the parameter names to work around this.
   *
   * @param string $query_string
   *   The url parameters as a string.
   *
   * @return array
   *   The url parameters as an array.
   *
   * @see https://www.drupal.org/project/drupal/issues/2984272
   */
  public static function parseQueryString($query_string = '') {
    $query_options = [];
    if ($query_string) {
      $encoded_string = preg_replace_callback('/(?:^|(?<=&))[^=[]+/', function ($match) {
        return bin2hex(urldecode($match[0]));
      }, $query_string);
      parse_str($encoded_string, $query_array);
      $query_options = array_combine(array_map('hex2bin', array_keys($query_array)), $query_array);
    }
